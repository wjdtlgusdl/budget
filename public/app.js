...
function shouldUseKnownCalcName(extractedName, knownName){
  const e = norm(extractedName || '');
  const k = norm(knownName || '');
  if(!k) return false;
  if(!e) return true;
  // PDF 줄바꿈으로 첫 글자가 빠진 산출내역명은 보정명을 우선합니다.
  if(/^(급보조비|구수당|통보조비|정액|장근로수당|직적립금|학차량)/.test(e)) return true;
  // 상여금 복잡 산식에서 산식 내부의 명절/행사/방학/스승의날 단어가 항목명처럼 잡히는 경우 보정합니다.
  if(/명절|행사|방학|스승/.test(e) && k === '상여금') return true;
  // knownName이 더 완전한 명칭이면 우선합니다.
  if(k.length >= e.length + 1 && (k.includes(e) || e.includes(k.slice(1)))) return true;
  return false;
}
...
function parseCalcLine(t){
  ...
  if(shouldUseKnownCalcName(extractedName, knownName)) extractedName = knownName;
...
function cleanItemName(name){
  ...
  // pdf.js가 항목명의 첫 글자를 앞 블록으로 떼어내는 경우 보정
  if(/^량기사/.test(s)) s = '차' + s;
  if(/^량보조/.test(s)) s = '차' + s;
  if(/^량주유/.test(s)) s = '차' + s;
  if(/^량운영/.test(s)) s = '차' + s;
  ...
  if(/^급보조비/.test(s)) s = '직' + s;
  if(/^구수당/.test(s)) s = '연' + s;
  if(/^통보조비/.test(s)) s = '교' + s;
  if(/^장근로수당/.test(s)) s = '연' + s;
  if(/^절휴가비/.test(s)) s = '명' + s;
  if(/^정액$/.test(s)) s = '정액급식비';
  return s;
}
...
function getPdfRetirementSummary(pdfItems){
  ...
  // 금액 비교는 하지 않지만 표시 금액은 실제 산출기초의 퇴직 항목을 우선합니다.
  // 총액행은 PDF 줄바꿈 보정 과정에서 그밖의인건비 등이 섞일 수 있어 보조로만 사용합니다.
